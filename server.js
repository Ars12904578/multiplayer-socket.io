...
let connectedPlayers = 0;let players = {};
io.on('connection', (socket) => {connectedPlayers++;
...
io.emit('message', `${disconnectedPlayerName} disconnected`);connectedPlayers--;
io.emit('playersCount', connectedPlayers);console.log(disconnectedPlayerName + " disconnected");
delete players[socket.id];}});});server.listen(PORT, () => {console.log(`Server is running on port ${PORT}`);});